...
func Calculate(n ast.Node) float64 {
	var v *HalstVol
	v = &HalstVol{
		Val: 0.0,
		Coef: Coef{
...
func (v *HalstVol) Visit(n ast.Node) ast.Visitor {
	v.Analyze(n)
	return v
}
...
func (v *HalstVol) Calc() float64 {
	...
	return float64(length) * math.Log2(float64(vocab))
}
